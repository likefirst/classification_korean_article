...
local hangul = "ㄱㄲㄳㄴㄵㄶㄷㄸㄹㄺㄻㄼㄽㄾㄿㅀㅁㅂㅃㅄㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎㅏㅐㅑㅒㅓㅖㅗㅘㅙㅚㅛㅜㅝㅞㅟㅠㅡㅢㅣ0123456789-,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]{} "

-- Training data
config.train_data = {}
config.train_data.file = paths.concat(paths.cwd(), "../article-data/train.t7b")
config.train_data.hangul = hangul
config.train_data.length = 1014
config.train_data.batch_size = 64

-- Validation data
config.val_data = {}
config.val_data.file =  paths.concat(paths.cwd(), "../article-data/test.t7b")
config.val_data.hangul = hangul
config.val_data.length = 1014
config.val_data.batch_size = 64

-- The model
config.model = {}
-- #hangul x 1014
config.model[1] = {module = "nn.TemporalConvolution", inputFrameSize = utf8.len(hangul), outputFrameSize = 256, kW = 7}
