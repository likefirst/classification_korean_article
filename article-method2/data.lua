...
function Data:getBatch(inputs, labels, data, extra)
   ...
   local inputs = inputs or torch.Tensor(self.batch_size, self.hangul_length + 1, self.length)
...
function Data:iterator(static, data)
   ...
   if static then
      inputs = torch.Tensor(self.batch_size, self.hangul_length + 1, self.length)
...
function Data:stringToTensor(str, l, input, p)
   ...
   local t = input or torch.Tensor(self.hangul_length + 1, l)
   t:zero()
   local j = 1
   for i = utf8.len(s), math.max(utf8.len(s) - l + 1, 1), -1 do
      if self.dict[utf8.sub(s,j,j)] then
          ...
          t[1][utf8.len(s) - i + 1] = math.floor(charLocation / self.hangul_length)
