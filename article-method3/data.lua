...
function Data:getBatch(inputs, labels, data, extra)
   ...
   local inputs = inputs or torch.Tensor(self.batch_size, self.binary_length, self.length)
...
function Data:iterator(static, data)
   ...
   if static then
      inputs = torch.Tensor(self.batch_size, self.binary_length, self.length)
...
function Data:stringToTensor(str, l, input, p)
   ...
   local t = input or torch.Tensor(self.binary_length, l)
   t:zero()
   local j = 1
